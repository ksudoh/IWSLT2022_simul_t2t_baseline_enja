...
model_dir=${model_basedir}/mma-il
devtest_dir=${devtest_basedir}/mma-il
...
for dataset in ${devtestset[@]} ; do
    env PYTHONPATH="${FAIRSEQ_ROOT}:${SIMULEVAL_ROOT}" \
        ${python3} ${SIMULEVAL_ROOT}/bin/simuleval \
            --agent ${FAIRSEQ_ROOT}/examples/simultaneous_translation/eval/agents/simul_trans_text_agent.py \
